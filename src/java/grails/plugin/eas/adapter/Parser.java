    ...
    public String getValue() throws IOException {
        ...
        if (type == END) {
            if (log.isDebugEnabled()) {
                log.debug("No value for tag: " + tagTable[startTag - TAG_BASE]);
            }
            return "";
        }
        // Save the value
    ...
    private final int getNext(boolean asInt) throws IOException {
        ...
        switch (id) {
            ...
            case Wbxml.STR_I:
                ...
                if (log.isDebugEnabled()) {
                    name = tagTable[startTag - TAG_BASE];
                    log.debug(name + ": " + (asInt ? Integer.toString(num) : text));
                }
                break;
